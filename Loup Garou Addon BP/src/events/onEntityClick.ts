import { Player, world, PlayerInteractWithEntityAfterEvent, system } from "@minecraft/server";
import { DB } from "misc/database";
import { formatRoleName, getGameInfos } from "misc/utils";

world.beforeEvents.playerInteractWithEntity.subscribe(e => {
  const player = e.player;
  const target = e.target as Player;

  if (!DB.game || target.typeId != "minecraft:player") return;

  system.run(() => {
    const powerName = player.getDynamicProperty("power");
    switch (powerName) {
      case "seer:see_role": seeRole(player, target); break;
      case "ryan:steal": steal(player, target); break;
      default: break;
    }
  })
})

function seeRole(player: Player, target: Player): void {
...
function steal(player: Player, target: Player) {
  ...
  for (let i = 0; i < targetInventory.size; i++) {
    ...
    if (!item || item.getDynamicProperty("isMenuItem")) continue;

    if (item) itemIndexes.push(i);
  }
