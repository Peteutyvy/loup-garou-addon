...
import { formatRoleName, getGameInfos, teleportPlayerToSpawn } from "misc/utils";

world.afterEvents.entityDie.subscribe(e => {
  const killer = e.damageSource.damagingEntity as Player;
  const target = e.deadEntity as Player;

  if (!DB.game|| !killer || !target || killer.typeId != "minecraft:player" || target.typeId != "minecraft:player") return;
...
export function manageKill(killer: Player, target: Player): void {
