import { Player, ServerMessageAfterEventSignal, world } from "@minecraft/server";
...
world.afterEvents.itemUse.subscribe(e => {
  ...
  if (!item || !item.getDynamicProperty("isMenuItem")) return;
...
function openPreGameForm(player: Player): void {
  const body = [
...
function openGameExplanationForm(player: Player): void {
  const body = [
...
function openGameForm(player: Player): void {
...
function openPowerForm(player: Player, power: RolePower): void {
  const playerGameInfos = getGameInfos(player);
