...
system.runInterval(() => {
    if (!DB.game || DB.game.borderRay < DB.settings.borderMinRay || !DB.settings.borderEnabled || !DB.game.isActive)
        return;
    if (DB.game.borderRay < DB.settings.borderMinRay)
        return;
    DB.game.borderRay -= borderReduceRate;
}, 20);
system.runInterval(() => {
    if (!DB.game || DB.game.borderRay < DB.settings.borderMinRay || !DB.settings.borderEnabled)
        return;
    ...
    for (const player of world.getAllPlayers()) {
        ...
        for (let i = 0; i < steps; i++) {
            ...
            overworld.runCommand(`particle minecraft:basic_flame_particle ${x} ${y} ${z}`);
        }
    }
}, 5);
